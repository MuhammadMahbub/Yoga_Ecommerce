    ...
    <div class="content-detached">
        <div class="content-body">
                    ...
                    <div class="col-sm-12">
                        <div class="ecommerce-header-items">
                            <div class="result-toggler">
                                <div class="search-results">{{ $classes->count() }} {{ __("results found") }}</div>
                            </div>
                            <div class="view-options d-flex">
                                <div class="button-group-spacing">
                                    @if (havePermission('class','create'))
                                        <a class="btn btn-warning waves-effect w-100 w-sm-auto" href="{{ route('yogaclass.create') }}">
                                            <i data-feather='plus'></i>
                                            <span class="pl-50">{{ __("Add New Class") }}</span>
                                        </a>
                                    ...
                                    <div class="dropdown d-inline-block w-100 w-sm-auto">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-right w-100">
                                            <a class="dropdown-item" href="{{ route('yogaclass.index') }}">All</a>
                                            <a class="dropdown-item {{ (request()->lowest == 'true') ? 'active' : '' }}" href="{{ route('yogaclass.index', 'lowest=true') }}">{{ __("Lowest") }}</a>
                                            <a class="dropdown-item {{ (request()->highest == 'true') ? 'active' : '' }}" href="{{ route('yogaclass.index', 'highest=true') }}">{{ __("Highest") }}</a>
                                        </div>
                                    </div>
            ...
            <section id="ecommerce-searchbar" class="ecommerce-searchbar">
                <div class="row mt-1">
                    <div class="col-sm">
                        <form action="{{ route('yogaclass.search') }}" method="get" class="input-group">
                    ...
                    @if (Route::is("yogaclass.search"))
                        <div class="col-sm-auto mt-50 mt-sm-0">
                            <a href="{{ route('yogaclass.index') }}" class="btn btn-danger waves-effect waves-float waves-light w-100 w-sm-auto">
            ...
            <section id="ecommerce-products" class="grid-view">
                @foreach ($classes as $class)
                    <div class="card ecommerce-card">
                        <a href="">
                            <img class="img-fluid card-img-top" src="{{ asset('uploads/class') }}/{{ $class->image }}" alt="{{ $class->title }}" />
                        </a>
                        <div class="card-body">
                            <div class="item-wrapper">
                                ...
                                {{-- <div>
                                    <h6 class="item-price">€{{ $class->price }}</h6>
                                </div> --}}
                            </div>
                            <h6 class="item-name">
                                <a class="text-body" href="">{{ $class->title }}</a>
                                <span class="card-text item-company">By <a href="javascript:void(0)" class="company-name">{{ $class->creator->name }}</a></span>
                            </h6>
                        ...
                        <div class="item-options text-center">
                            <div class="item-wrapper">
                                <div class="item-cost">
                                    <h4 class="item-price">€{{ $class->price }}</h4>
                                </div>
                            </div>
                            <div class="item-options__btn-wrapper">
                                @if (havePermission('class','edit')) 
                                    <a href="{{ route('yogaclass.pricing', $class->id) }}" class="btn btn-primary waves-float waves-light w-100" >
                                        <i data-feather='shopping-cart'></i>
                                        <span>{{ __("Pricing") }}</span>
                                    </a>  
                                    <a href="{{ route('class.gallery.index', $class->id) }}" class="btn btn-info waves-effect waves-float waves-light w-100">
                            ...
                            <div class="item-options__btn-wrapper">
                                @if (havePermission('class','edit')) 
                                    <a href="{{ route('yogaclass.edit', $class->id) }}" class="btn btn-warning waves-float waves-light w-100">
                                        <i data-feather='edit-2'></i>
                                        <span>{{ __("Edit") }}</span>
                                    </a>
                                @endif
                                @if (havePermission('class','delete'))
                                    <form action="{{ route('yogaclass.destroy') }}" method="POST" class="w-100">
                                        @csrf
                    ...
                    @push('all_modals') 
                        <!-- Modal -->
                        <div class="modal fade" id="classPricing{{ $class->id }}" tabindex="-1"  aria-hidden="true">
            ...
            @include('includes.pagination' , ['variables' => $classes, 'data_variables' => ['search' => request()->search, 'active' => request()->active,'lowest' => request()->lowest,'highest' => request()->highest]])
        ...
        <div class="modal fade" id="banner_setting" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <form action="{{ route('yogaClass.banner.update',$banner->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="modal-body">
