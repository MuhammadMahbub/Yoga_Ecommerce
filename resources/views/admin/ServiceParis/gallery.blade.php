...
{{-- Title --}}
@section('title')
    {{ config('app.name') }} | {{ __("Service Class") }}
@endsection
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header">
                <h4 class="card-title">{{ __("Service Paris gallery") }}</h4>
                        ...
                        <tbody>
                            @foreach ($galleries as $gallery)
                                <tr>
                                    <td>{{ $loop->index + 1 }}</td>
                                    <td>
                                        <img src="{{ asset('uploads/service_gallery') }}/{{ $gallery->image }}" alt="Image" height="150">
                                    </td>
                                    ...
                                    <td>
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-right">
                                                ...
                                                <form action="{{ route('service.gallery.delete', $gallery->id) }}" method="POST">
                                                    @csrf 
                                    ...
                                    <div class="modal fade" id="edit_modal{{ $gallery->id }}" tabindex="-1" aria-hidden="true">
                                        <div class="modal-dialog modal-dialog-centered">
                                            <div class="modal-content">
                                                ...
                                                <form action="{{ route('service.gallery.update',$gallery->id) }}" method="POST" enctype="multipart/form-data">
                                                    @csrf 
                ...
                <div class="modal fade" id="add_new" tabindex="-1" aria-hidden="true">
                    <div class="modal-dialog modal-dialog-centered">
                        <div class="modal-content">
                            ...
                            <form action="{{ route('service.gallery.store') }}" method="POST" enctype="multipart/form-data">
                                @csrf 
...
@push('js')
    <script>
        ...
        $('body').on('click','.order_update_btn',function (e) { 
            ...
            $.ajax({
                url: "{{ route('service.gallery.order.update') }}",
                type: "post",
                data: {
                    id:id,
                    order:order,
