    ...
    <div class="eventsWrap">
        @foreach ($classes as $class)
            <div class="eventItem clear">
                <a href="{{ route('class.details', [\Str::slug($class->title), $class->id]) }}" class="eventItemImg">
                    <img src="{{ asset('uploads/class')}}/{{ $class->image }}" alt="Thumbnail">
                </a>
                <div class="eventItemDesc">
                    ...
                    {{-- <h3>{{ $class->price }}€</h3> --}}
                    <h3><a href="{{ route('class.details', [\Str::slug($class->title), $class->id]) }}">{{ $class->title }}</a></h3>
                    <p>{{ $class->short_description }}</p>
                    <a href="{{ route('class.details', [\Str::slug($class->title), $class->id]) }}" class="eventLearnMore">learn more
                        <i>
